...
function BadgeAvatar({ size }: { size: BadgeSize }) {
  return (
    <Avatar size={AVATAR_SIZE_FOR_BADGE[size]} className="after:hidden">
      <AvatarImage src={BADGE_AVATAR.src} alt={BADGE_AVATAR.alt} />
      <AvatarFallback className="text-[8px] leading-none">{BADGE_AVATAR.fallback}</AvatarFallback>
    </Avatar>
  )
}
  ...
  return (
    <div className="space-y-0 divide-y rounded-xl border border-border/60 bg-muted">
      ...
      <div className="px-4 py-5">
        <Table>
          <TableCaption className="caption-top mb-3 text-left text-xs font-medium text-muted-foreground">
            Avatar badge — leading avatar scales with size
          </TableCaption>
          <TableHeader>
            <TableRow>
              <TableHead className="w-[8.5rem]">Size</TableHead>
          ...
          <TableBody>
            {BADGE_SIZES.map(({ token, heightPx }) => (
              <TableRow key={token}>
                ...
                {BADGE_VARIANTS.map(({ token: variant }) => (
                  <TableCell key={variant}>
                    <Badge variant={variant} size={token}>
                      <BadgeAvatar size={token} />
                      {BADGE_AVATAR.name}
                      {showTrailingIcon ? <Plus data-icon="inline-end" aria-hidden /> : null}
                    </Badge>
                  </TableCell>
